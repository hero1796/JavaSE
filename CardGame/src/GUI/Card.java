...
public class Card extends JButton {
    ...
    private void processImage() {
        if (this.cardData.getSuit() <= 1) {
            this.suitImge = SuitImages.getSpade();
        } else if (this.cardData.getSuit() == 2) {
            this.suitImge = SuitImages.getHeart();
        } else if (this.cardData.getSuit() == 3) {
            this.suitImge = SuitImages.getTref();
        } else if (this.cardData.getSuit() >= 4) {
            this.suitImge = SuitImages.getDiamond();
        }
    }

    private void processLable() {
        if (cardData.getValue() == 14) {
            this.lbl = "A";
        } else if (cardData.getValue() == 11) {
            this.lbl = "J";
        } else if (cardData.getValue() == 12) {
            this.lbl = "Q";
        } else if (cardData.getValue() == 13) {
            this.lbl = "K";
        } else {
            this.lbl = "" + cardData.getValue();
        }
    }
